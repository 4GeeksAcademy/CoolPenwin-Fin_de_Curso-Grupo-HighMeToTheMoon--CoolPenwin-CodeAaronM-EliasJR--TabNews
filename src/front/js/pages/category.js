...
export const Category = () => {
    ...
    const handleCategorySubmit = async (e) => {
        ...
        if (editMode) {
            ...
            const updatedCategories = store.categories.map(category => 
                category.id === currentCategoryId ? { ...category, ...newCategory } : category
            );
            actions.setCategories(updatedCategories); // Suponiendo que tienes un método para establecer categorías en el store
        } else {
            const createdCategory = await actions.newCategory(newCategory);
            // Agrega la nueva categoría al estado local
            actions.setCategories([...store.categories, createdCategory]); // Asumiendo que newCategory regresa el objeto creado con ID
        }
    ...
    const handleDeleteCategory = async (id) => {
        ...
        // Filtra la categoría eliminada del estado local
        const updatedCategories = store.categories.filter(category => category.id !== id);
        actions.setCategories(updatedCategories); // Actualiza el estado local
    };
                ...
                <tbody>
                    {store.categories.map((category) => (
                        <tr key={category.id}>
                            ...
                            <td>
                                <Button variant="warning" onClick={() => openEditModal(category)}>
                                    Edit
                                </Button>
                                <Button variant="danger" onClick={() => handleDeleteCategory(category.id)} className="mx-2">
                                    Delete
                                </Button>
                            </td>
                        </tr>
